...
object ReunionAnalysisPrompt {
    ...
    fun buildUserPrompt(input: AnalysisInput): String {
        return buildString {
            ...
            appendLine("nextStep: one gentle next direction under 90 Korean characters. Avoid commanding tone.")
            appendLine("messageDraft: if contactReadiness is 지금은 보류 or 정보 부족, say contact can rest today or missing information can be checked. Otherwise one gentle copyable line under 70 Korean characters.")
            appendLine("alternativeDrafts: exactly 3 short candidate messages or rest/check actions separated by newline characters.")
            appendLine("caution: one practical boundary under 90 Korean characters, phrased calmly.")
            appendLine()
