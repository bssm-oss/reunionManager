...
- Gemma 모델 파일이 없을 때도 전체 흐름을 확인할 수 있는 기본 정리 모드
...
- Gemma 4 모델이 없거나 실행 점검 전이면 기본 정리 provider를 사용합니다.
...
- 로컬 AI 설정에서 모델 파일을 제거하거나 다른 모델로 교체하면 앱 전용 저장소의 기존 복사본만 정리하고 기본 정리로 돌아갈 수 있습니다.
